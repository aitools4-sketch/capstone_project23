  ...
  return (
    <section id="top" className="relative overflow-hidden py-16 sm:py-20">
      {/* No Reveal wrapper here on purpose: the Hero is always the first
          ...
          min-h-[calc(100dvh_-_4.5rem)]: fills the rest of the first screen
          below the sticky Navbar (~4.5rem tall: py-4 padding + its tallest
          row content + border, see Navbar.tsx) and centers its content
          within that space via flex, instead of the old fixed top-padding
          scheme (pt-16 → lg:pt-40). That approach positioned the hero
          content in proportion to *breakpoint*, not actual viewport
          height, so short/tall viewports at the same breakpoint got
          visibly unbalanced spacing — a lot of empty space above the
          content and very little below, or vice versa. dvh (not vh) so
          mobile browsers with a collapsing address bar don't overshoot.
          py-16/py-20 above is just a floor for very short viewports
          (landscape phones) — min-height never lets content get clipped,
          it only ever adds room if the content itself needs more. */}
      <div className="mx-auto flex min-h-[calc(100dvh_-_4.5rem)] max-w-4xl flex-col items-center justify-center px-6 text-center">
